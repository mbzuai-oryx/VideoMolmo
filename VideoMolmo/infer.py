...
def get_output(images, prev_frames, prompt, model=None, processor=None, tokenizer=None, device='cuda'):
    conversation = [
...
def load_models(args, device='cuda'):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.molmo_model)
    ...
    return model, tokenizer, processor, predictor

def main(args):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model, tokenizer, processor, predictor = load_models(args, device=device)
    ...
    for f in range(0, total_frames, frame_rate):
        ...
        image = pil_images[f]
        outputs = get_output([image], prev_frames, args.prompt, model=model, processor=processor, tokenizer=tokenizer, device=device)
...
def parse_args():
    ...
    parser.add_argument("--model_path", type=str, default='ghazishazan/VideoMolmo')
