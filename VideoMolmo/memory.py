...
def validate_labels(labels, vocab_size, ignore_index=-100):
    """Check that all labels (except those equal to ignore_index) are in [0, vocab_size - 1]."""
    ...
    if valid_mask.any():
        ...
        if min_label < 0 or max_label >= vocab_size:
            raise ValueError(
                f"Labels out of range: found values in range [{min_label}, {max_label}] "
                f"but expected in [0, {vocab_size - 1}] (ignoring index {ignore_index})."
            )
...
class OLMoPretrainedVisionBackbone(OLMoVisionBackbone):
    ...
    def encode_image(self, images: torch.Tensor) -> torch.Tensor:
        ...
        # print('>> image_features.shape', image_features.shape) # (batch_size, num_crops, num_patch, d_model) [1, 13, 576, 2048]
        # print('>> cls_embed.shape', cls_embed.shape) # (batch_size, num_crops, d_model) [1, 13, 2048]
    ...
    def forward(self, images: torch.Tensor, prev_frames: torch.Tensor, image_masks: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        for i in range(num_frames):
            with torch.no_grad():
                f, _ = self.encode_image(prev_frames[:, i],)
            if running_mean is None:
                running_mean = f
            else:
                # update mean in place: new_mean = old_mean + (f - old_mean)/(i+1)
                running_mean = running_mean + (f - running_mean) / (i + 1)
        prev_features = running_mean
        # MODIFY
...
class Molmo(nn.Module):
    ...
    def forward(
        ...
        if images is not None:
            ...
            image_features = image_features.view(batch_size, num_image * num_patch, -1) # (batch_size, num_image * num_patch, d_model)
            image_input_idx = image_input_idx.view(batch_size, num_image * num_patch) # (batch_size, num_image * num_patch)
...
class MolmoForCausalLM(PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            if loss_masks is not None:
                ...
                # Validate labels for the shifted logits.
                vocab_size = shift_logits.size(-1) #TODO
                validate_labels(shift_labels, vocab_size, ignore_index=-100) #TODO
    ...
    def generate_from_batch(
        ...
        **kwargs,
        ...
        prev_frames = batch.get("prev_frames")
    ...
    def prepare_inputs_for_generation(
        ...
        if self.config.use_position_ids:
            ...
            prev_frames = kwargs.get("prev_frames")
            ...
            if past_key_values is None:
                ...
                model_inputs["prev_frames"] = prev_frames
    ...
    def _update_model_kwargs_for_generation(
        ...
        if self.config.use_position_ids:
            ...
            if "prev_frames" in model_kwargs:
                del model_kwargs["prev_frames"]
