...
from MolmoVideo.molmo_video.memory_preprocessor import MultiModalPreprocessor
from MolmoVideo.molmo_video.memory import MolmoForCausalLM  
...
for idx in tqdm(range(len(videos))):
    ...
    frames_dir = os.path.join(root, 'frames', video)
    if not os.path.isdir(frames_dir):
        print(f"Warning: Missing frames directory for video {video}")
        continue
    ...
    prompt = f"{annotations[video]['caption']}"
